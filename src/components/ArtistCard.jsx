...
export default function ArtistCard({ artist }) {
  ...
  return (
    <Link href={artistLink} className="block group">
      <Card
        className="bg-surface-container-low dark:bg-inverse-surface/40 rounded-2xl border border-outline-variant/20 dark:border-outline-variant/10 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer"
        shadow="none"
      >
        <div className="flex flex-col items-center text-center p-6">
          ...
          <p className="text-lg font-semibold text-on-surface dark:text-inverse-on-surface group-hover:text-primary dark:group-hover:text-primary-fixed-dim transition-colors leading-snug">
            {artist.name}
          </p>
          ...
          <div className="mt-4 inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-secondary-container/80 dark:bg-secondary/20 text-on-secondary-container dark:text-secondary-fixed text-sm font-medium">
            ...
            <span>{artist.salesCount ?? artist.sales ?? 0} Sales</span>
          </div>
        </div>
      </Card>
    </Link>
